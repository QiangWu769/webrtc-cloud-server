    ...
    cat > webrtc_config_results/sender_config_local.json << EOF
{
  "video_source": {
    ...
    "video_file": {
      "enabled": true,
      "file_path": "$(pwd)/VCD_th_1920x1080_30.yuv",
...
EOF
    ...
    echo "4️⃣ 启动本地发送端..."
    ./src/out/Default/peerconnection_client --config=webrtc_config_results/sender_config_local.json > webrtc_config_results/sender_local.log 2>&1 &
    CLIENT_PID=$!
...
EOF
    ...
    echo "4️⃣ 启动本地接收端..."
    ./src/out/Default/peerconnection_client --config=webrtc_config_results/receiver_config_local.json > webrtc_config_results/receiver_local.log 2>&1 &
    CLIENT_PID=$!
