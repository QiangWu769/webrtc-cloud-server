...
int64_t ClockSyncHelper::RemoteToLocalTime(int64_t remote_time_ms) {
  ...
  // remote_time is in remote's clock
  // Convert to NTP reference: remote_time - remote_offset
  // Then to local clock: + local_offset
  // Net effect: remote_time - (remote_offset - local_offset)
  //           = remote_time - remote_offset + local_offset
  //           = remote_time + (local_offset - remote_offset)
