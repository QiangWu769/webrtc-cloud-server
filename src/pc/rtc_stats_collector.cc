 ...
 */
 ...
 // Helper function to emit wall clock timestamp with microsecond precision
 // so logs can be correlated with external systems (e.g., diag BSR pipeline).
 static inline std::string GetWallClockTimestampString() {
   double unix_seconds = TimeUTCMillis() / 1000.0;
   std::stringstream ss;
 ...
 void SetInboundRTPStreamStatsFromMediaReceiverInfo(
   ...
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                    << "[VideoQuality-Bitrate] MonoTime: " << TimeMillis()
                    << ", SSRC: " << media_receiver_info.ssrc()
 ...
 CreateInboundRTPStreamStatsFromVideoReceiverInfo(
   ...
   // **添加丢包日志 - 直接的网络问题指标**
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                    << "[VideoQuality-PacketLoss] MonoTime: " << TimeMillis()
                    << ", SSRC: " << video_receiver_info.ssrc()
                    << ", Packets Lost: " << video_receiver_info.packets_lost;
   ...
   // **添加网络抖动日志 - 衡量网络质量稳定性**
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                    << "[VideoQuality-Jitter] MonoTime: " << TimeMillis()
                    << ", SSRC: " << video_receiver_info.ssrc()
                    << ", Jitter (ms): " << video_receiver_info.jitter_ms;
   ...
   if (video_receiver_info.framerate_decoded > 0) {
     inbound_video->frames_per_second = video_receiver_info.framerate_decoded;
       ...
       // **添加帧率日志 - 按照论文规范**
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                    << "[VideoQuality-FrameRate] MonoTime: " << TimeMillis()
                    << ", SSRC: " << video_receiver_info.ssrc()
   ...
   if (video_receiver_info.qp_sum.has_value()) {
     ...
     // **添加量化参数日志 - 衡量视频编码质量**
     double avg_qp = (video_receiver_info.frames_decoded > 0) 
                     ? (static_cast<double>(*video_receiver_info.qp_sum) / video_receiver_info.frames_decoded) 
                     : 0.0;
     RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                      << "[VideoQuality-QP] MonoTime: " << TimeMillis()
                      << ", SSRC: " << video_receiver_info.ssrc()
                      << ", QP Sum: " << *video_receiver_info.qp_sum
                      << ", Average QP: " << avg_qp;
   }
   ...
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "] "
                    << "[VideoQuality-FreezeRate] MonoTime: " << TimeMillis()
                    << ", SSRC: " << video_receiver_info.ssrc()
